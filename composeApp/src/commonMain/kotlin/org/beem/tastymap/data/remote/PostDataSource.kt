...
class PostDataSource(private val authHttpClientManager: AuthHttpClientManager) {
    ...
    suspend fun addPost(request: PostAndVisitRequest):PostResponse {
        return client.patch("api/posts/add") {
            setBody(request)
        }.body()
    }

    suspend fun getUserPosts(userId: Long, page: Int = 0, size: Int = 12): PageResponse<PostResponse> {
    ...
    suspend fun getMyPosts(page: Int = 0, size: Int = 10): PageResponse<PostResponse> {
